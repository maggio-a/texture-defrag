...
{
private:
  enum KnownTypes { KT_UNKNOWN, KT_PLY, KT_STL, KT_OFF, KT_OBJ, KT_VMI, KT_FBX };
static int &LastType()
{
...
static int Open(OpenMeshType &m, const char *filename, int &loadmask, CallBackPos *cb=0)
{
    ...
    else if(FileExtension(filename,"vmi"))
    {
        ...
        else if (FileExtension(filename, "fbx")) {
                err = ImporterFBX<OpenMeshType>::Open(m, filename, cb);
                loadmask = ImporterFBX<OpenMeshType>::LoadMask(filename);
                LastType() = KT_FBX;
        }
  else {
		err=1;
...
static bool ErrorCritical(int error)
{
  switch(LastType())
  {
    ...
    case KT_FBX : return (error>0); break;
  }
...
static const char *ErrorMsg(int error)
{
  switch(LastType())
  {
    ...
    case KT_FBX : return ImporterFBX<OpenMeshType>::ErrorMsg(error); break;
  }
  return "Unknown type";
}
...
static bool LoadMask(const char * filename, int &mask)
{
	...
	else if(FileExtension(filename,"obj"))
	{
        ...
        {
                err = ImporterFBX<OpenMeshType>::LoadMask(filename, mask);
                LastType() = KT_FBX;
        }
	else
	{
